...
for /f %%i in ('"bin/adb/adb.exe" shell "ls -d %ROOT_DIR_DB%/*.default 2>/dev/null" ^|^| "bin/sed.exe" "s/default.*$/default/g"') do set DB_DIR=%%i
@echo %DB_DIR%
...
@echo Getting %CERT%
"bin/adb/adb.exe" pull %DB_DIR%/%CERT% ./%TEMP%/

@echo Getting %KEY%
"bin/adb/adb.exe" pull %DB_DIR%/%KEY% ./%TEMP%/

@echo Getting %PKCS11%
"bin/adb/adb.exe" pull %DB_DIR%/%PKCS11% ./%TEMP%/
...
:: Push files to phone
@echo Stopping B2G
"bin/adb/adb.exe" shell stop b2g

@echo copying %CERT%
"bin/adb/adb.exe" push ./%TEMP%/%CERT% %DB_DIR%/%CERT%
@echo copying %KEY%
"bin/adb/adb.exe" push ./%TEMP%/%KEY% %DB_DIR%/%KEY%
@echo copying %PKCS11%
"bin/adb/adb.exe" push ./%TEMP%/%PKCS11% %DB_DIR%/%PKCS11%

@echo Starting B2G
"bin/adb/adb.exe" shell start b2g
